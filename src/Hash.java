...
public class Hash {
    ...
    static void textMode(String a) {
        Scanner input = new Scanner(System.in);
        // System.out.println("Enter your text: ");
        String entered = a;// input.nextLine();
    ...
    static void fileMode(String a) {
        Scanner input = new Scanner(System.in);
        // System.out.println("Enter file name: ");
        String name = a;// input.nextLine();
        byte[] array = {};
        try {
        ...
        int[] tm = new int[array.length];
        for (int i = 0; i < array.length; i++) {
            tm[i] = array[i] & 0xff;
        }
        chunks = new String[(int) ((double) tm.length / 64.0 + 1)][64];
        for (int i = 0; i < chunks.length; i++) {
            for (int j = 0; j < 64; j++) {
                if (i*64+j >= tm.length) {
                    if (i*64+j == tm.length) {
                    ...
                    String temp = Integer.toBinaryString(tm[i*64+j]);
                    String temp2 = "";
                    if (temp.length() < 8) {
                        for (int k = 0; k < 8-temp.length(); k++) {
                            temp2 += "0";
